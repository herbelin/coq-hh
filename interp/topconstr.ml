...
let rec match_ inner alp (tmetas,blmetas as metas) sigma a1 a2 =
  ...
  | b1, ALambda (Name id,AHole _,b2) when inner ->
      let id' = Namegen.next_ident_away id (free_rawvars b1) in
      match_ true alp metas (bind_binder sigma id
       [(Name id',Explicit,None,RHole(dummy_loc,Evd.BinderType (Name id')))])
       (mkRApp dummy_loc b1 (RVar (dummy_loc,id'))) b2
